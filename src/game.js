(() => {
  ...
  function refreshCareer() {
    ...
    for (const contract of ContractSystem.contracts) {
      const completed = career.completed.includes(contract.id);
      document.getElementById(`contract-${contract.id}`).textContent = `${completed ? 'REPLAY' : 'ACCEPT'} CONTRACT`;
      document.getElementById(`completed-contract-${contract.id}`).hidden = !completed;
      document.getElementById(`select-contract-${contract.id}`).classList.toggle('contract-completed', completed);
    }

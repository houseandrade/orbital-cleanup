...
const sandbox = {
  ...
  caches: { open: async () => ({ put: async (key) => { stored.push(key); } }),
    keys: async () => ['orbital-cleanup-v0.8.1', 'other-app-cache', 'orbital-cleanup-v0.12-salvage2'],
...
await work;
assert.ok(requests.every(({ url, options }) => url.includes('build=orbital-cleanup-v0.12-salvage2') && options.cache === 'reload'));
assert.ok(stored.includes('./src/input.js?v=0.12-salvage2'), 'fresh versioned controls cached offline');
